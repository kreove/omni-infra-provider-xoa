...
The first Omni-driven cluster provisioning also found that **Xen Orchestra's own cloud-init config drive is unreadable by Talos**. Its contents are correct — join config, meta-data and network-config, on a filesystem labelled `cidata` — but XO wraps the FAT16 in an MBR whose only partition starts at LBA 1, which is ambiguous with a whole-disk filesystem. Talos's block discovery rejects the table and then finds only the MBR at offset 0, so it reports the drive as a bare disk with no type and no label, cannot find the `cidata` volume, and stays in maintenance mode holding a join config it cannot see. XO exposes no option to change the layout, so the provider builds the drive itself (`internal/pkg/provider/cidata.go`) and attaches it with `vm.attachDisk` before the first power-on. Verified live: a drive with byte-identical content in the corrected layout joined Omni immediately.

The same provisioning run found the most serious bug of the port: **VMs were created with dynamic memory left at the seed template's 256 MiB while static-max was set to the Machine Class size**, which makes XAPI boot the guest in Xen populate-on-demand mode. Talos touches memory immediately and runs no balloon driver, so Xen crashed every VM within seconds of power-on — before the kernel console existed, so nothing was visible in the guest at all. The failure only appears in `xl dmesg` on the host (`p2m_pod_demand_populate: out of PoD memory`). The fix pins dynamic-min = dynamic-max = static-max at creation. Notably, everything else in the provisioning path had been verified byte-for-byte correct (image content, partition tables, bootloader, kernel command line, firmware) before the actual cause was found in the hypervisor log — see [Troubleshooting](troubleshooting.md#vm-boots-console-goes-blank-then-the-vm-cycles-running--paused--halted).
...
- `vm.attachDisk` for the config drive, and VDI upload over the REST API
